...
void EEPROM_readPage(uint16_t addr, uint8_t* dataRX, uint8_t nBytes) {
	...
	/* Read the nBytes */
	SPI_Interface_Multi_RW(dataTX, 3, dataRX, nBytes);//leggiamo nbYtes e li mettiamo in dataRX
...
void EEPROM_writePage(uint16_t addr, uint8_t* data, uint8_t nBytes) {
    ...
    /* Enable WRITE operations */
    EEPROM_writeEnable();//funzione già implementata
    ...
    */
	uint8_t dataTX[3+nBytes]; //byte istruzione + 2 indirizzo + dati veri e propri
    dataTX[0] = SPI_EEPROM_WRITE;
    dataTX[1] = (addr & 0xFF00) >> 8;
    dataTX[2] = addr & 0x00FF;
    /* Copy the input data in the memory */
	memcpy(&dataTX[3], data, nBytes);//copia di dati da un'array (data) a un altro (daataTX da indirizzo 3 -> i primi tre sono già occupati)
	
	/* Nothing to RX: point to a dummy variable */
	uint8_t temp = 0;
	
	SPI_Interface_Multi_RW(dataTX, 3+nBytes, &temp, 0);//non vogliamo leggere nulla, solo trasmettere
